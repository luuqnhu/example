...
class UserRestController extends Controller
{
    ...
    public function userLoginAction(){
        ...
        $username = $array["username"];
        $password = $array["password"];
        $result = new FormatResponse();
        $userLogic = new UserLogic($this->get('aws.dynamodb'));
        $user = $userLogic->UserLogin($username, $password);

        if($user === FALSE){
            $response = $result->signInByUsernameResponse($common->RESULT_CODE_FAIL, $common->LOGIN_FAIL, null, null );
        }
        else {
            $token = uniqid();
            $resultToken = $tokenLogic->insertNewToken($username, $token, date('Y-m-d H:i:s'));
            if($resultToken === FALSE) {
                return $result->signInByUsernameResponse($common->RESULT_CODE_FAIL, $common->LOGIN_FAIL, null, null);
            }
            $response = $result->signInByUsernameResponse($common->RESULT_CODE_SUCCESS, $common->LOGIN_SUCCESSFULLY, $user, $token);
    ...
    public function userLogoutAction() {
        ...
        if($username === FALSE)
            ...
            $response = $tokenLogic->deleteToken($username);
            if($response === FALSE)
                return $formatResponse->createResponseRegister($common->RESULT_CODE_FAIL, $common->USER_LOGOUT_FAIL);
